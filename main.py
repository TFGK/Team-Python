...
pts2 = [] #배열
fn = r"./data/test.mp4"
fn_yaml = r"./data/parking_sapce_data.yml"
...
class test(QMainWindow):
    ...
    def initUI(self):           #기본 UI
        ...
        pybutton = QPushButton('종료', self)
        pybutton.resize(100, 30)
        pybutton.move(130 ,60)
        pybutton.clicked.connect(QCoreApplication.instance().quit)

        self.setGeometry(700, 400, 360, 100)
    ...
    def space(self):
        ...
        cv2.namedWindow('image')    # 윈도우를 고정으로 할 것인지 사용자가 크기 조절 할 수  있는지
        ...
        while(cap.isOpened()):
            ...
            if config['parking_detection']:
                ...
                for ind, park in enumerate(parking_data):
                    ...
                    points[:,0] = points[:,0] - rect[0] # contour를 roi로 옮김
            ...
            if config['text_overlay']:
                cv2.rectangle(frame_out, (1, 5), (50, 10),(255,255,255)) 
                str_on_frame = "Spot: %d Occupied: %d" % (spot, occupied)
                cv2.putText(frame_out, str_on_frame, (1,20), cv2.FONT_HERSHEY_TRIPLEX,
                                0.7, (0,0,255), 2, cv2.LINE_AA)
                                
            # 비디오 프레임
            if config['save_video']:
                if video_cur_frame % 35 == 0: # 30 프레임마다 적용
                    out.write(frame_out)
